...
ShortestTimeToCompletion::ShortestTimeToCompletion(PCBController* pcbController) :
    ProcessScheduler(pcbController)
{
    //handled in init list
}

void ShortestTimeToCompletion::sortProcesses()
{
    //No need to sort
}

void ShortestTimeToCompletion::runProcesses()
{
    ...
    std::string fileName = "ShortestTimeToCompletionResults.txt";
    ...
    while(getPCBController() -> getReadyQueue() -> getHead() != NULL || getPCBController() -> getRunningProcess() != NULL)
    {
        if(pcbToRun == NULL)
        {
            pcbToRun = getPCBController() -> findShortestPCB(currentRunningTime);
        }
...
/**
  ...
  */
        ...
        if(getPCBController() -> findShortestPCB(currentRunningTime) != pcbToRun)
        {
            ///Process Details
